...
void receipt_clear(void) {
    if (internal_clear_ptr!=NULL) obstack_free(&internal_receipt_pool, internal_clear_ptr);
    internal_clear_ptr=NULL;
    internal_first_record=NULL;
    internal_last_record=NULL;
    internal_records_count=0;
...
struct record_receipt * receipt_newRecord(void) {
    ...
    num_init(&rec->summ, 2);
    if (internal_clear_ptr==NULL) internal_clear_ptr=rec;
    return rec;
    }
...
void receipt_showAll(void) {
    ...
    while (rc!=NULL) {
	num_addThis(&global_total_summ, &rc->summ);
